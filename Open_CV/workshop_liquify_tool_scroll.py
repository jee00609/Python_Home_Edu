...
def onMouse(event,x,y,flags,param):     
    ...
    global half
    if event == cv2.EVENT_MOUSEMOVE:  # 마우스 중심 점을 기준으로 대상 영역 따라다니기
        if not isDragging :
            img_draw = img.copy()       
            cv2.rectangle(img_draw, (x-half, y-half), (x+half, y+half), (0,255,0)) # 드래그 영역 표시
        ...
        if isDragging:
            ...
            liquify(img, cx1, cy1, x, y)    # 드래그 시작 좌표와 끝난 좌표로 리퀴파이 적용 함수 호출
            cv2.imshow(win_title, img)
    elif event == cv2.EVENT_MOUSEWHEEL:     #휠 스크롤
        if flags > 0:
            half = half + 5
            img_draw = img.copy()       
            cv2.rectangle(img_draw, (x-half, y-half), (x+half, y+half), (0,255,0)) # 드래그 영역 표시
